...
");
        ...
        // Preinstall scripts are generated as a separate chocolateyBeforeModify.ps1
        // file (see CreateChocolateyBeforeModifyScript) so that Chocolatey and
        // sbin-installer can run them as a distinct pre-install phase rather than
        // as an anonymous preamble buried inside chocolateyInstall.ps1.
    ...
    /// <summary>
    /// Creates <c>chocolateyBeforeModify.ps1</c> from the project's <c>preinstall*.ps1</c>
    /// scripts. This file is the Chocolatey-standard hook that fires before any
    /// package modification (upgrade or uninstall), making it the correct home for
    /// pre-install logic in a .nupkg.
    ///
    /// <para><b>Chocolatey limitation:</b> <c>chocolateyBeforeModify.ps1</c> only runs
    /// when an <em>existing</em> package is being upgraded or uninstalled — it does
    /// <em>not</em> fire on a fresh install. This is a Chocolatey engine limitation,
    /// not a cimipkg design choice. Packages consumed by
    /// <see href="https://github.com/windowsadmins/sbin-installer">sbin-installer</see>
    /// do not have this limitation — sbin-installer executes
        ...
        IDictionary<string, string> envVars)
    {
        ...
        if (preinstallScripts.Count == 0)
        {
            return; // No preinstall scripts → no file generated.
        }

        var sb = new StringBuilder();

        sb.AppendLine(@"# chocolateyBeforeModify.ps1 - Generated by cimipkg
# Runs before the package is upgraded or uninstalled.
#
# NOTE: Chocolatey only fires this hook when an existing version is being
# modified (upgrade/uninstall). On a fresh install, this script is NOT
# executed by choco. sbin-installer does not have this limitation and
# always runs this script before install.
$ErrorActionPreference = 'Stop'
...
");
        ...
        foreach (var preinstallScript in preinstallScripts)
        {
            ...
            sb.AppendLine(content);
            sb.AppendLine();
            _logger.LogDebug("Included preinstall script in BeforeModify: {Script}",
                Path.GetFileName(preinstallScript));
        }

        var beforeModifyPath = Path.Combine(toolsDir, "chocolateyBeforeModify.ps1");
        File.WriteAllText(beforeModifyPath, sb.ToString(), Encoding.UTF8);
        _logger.LogInformation("Created chocolateyBeforeModify.ps1");
    }
